...
void main(List<String> args) async {
  ...
  final Map<int, List<String>> testPrints = {};
  ...
  process.stdout
    ...
    try {
      ...
      if (type == 'suite') {
        ...
        if (stackTrace != null) {
            final firstThreeLines = stackTrace.toString().split('\n').take(3).join('\n            ');
            errOutput += '\n            ' + firstThreeLines;
        }

        if (testErrors.containsKey(testId)) {
            testErrors[testId] = testErrors[testId]! + '\n' + errOutput;
        } else {
            testErrors[testId] = errOutput;
        }
      } else if (type == 'print') {
        final message = data['message'];
        final testId = data['testID'];
        if (testId != null) {
          testPrints.putIfAbsent(testId, () => []).add(message.toString());
        }
        print('         \x1B[36mℹ\x1B[0m \x1B[90m$message\x1B[0m');
        ...
        if (result != 'success') {
          String finalErr = testErrors[testId] ?? '';
          if (testPrints.containsKey(testId)) {
            finalErr += '\n' + testPrints[testId]!.join('\n');
          }
          test['finalError'] = finalErr;
        }
  ...
  void printCategory(String catName, List<TestFile> files, String type) {
     ...
     for (final f in files) {
        ...
        for (final c in cases) {
           ...
           if (c.error != null) {
               // Only take important lines from collated error
               final lines = c.error!.split('\n');
               final importantLines = lines.where((l) => 
                   l.contains('Expected:') || 
                   l.contains('Actual:') || 
                   l.contains('Which:') || 
                   l.contains('EXCEPTION CAUGHT') || 
                   l.contains('TestFailure') ||
                   l.contains('Finder:') ||
                   l.contains('Found 0 widgets')
               ).take(15).toList();
               
               if (importantLines.isEmpty) {
                   print('         \x1B[41m\x1B[37m ERROR \x1B[0m \x1B[31m${lines.take(5).join('\n         ')}\x1B[0m');
               } else {
                   print('         \x1B[41m\x1B[37m ERROR \x1B[0m \x1B[31m${importantLines.join('\n         ')}\x1B[0m');
               }
           }
        }
     }
