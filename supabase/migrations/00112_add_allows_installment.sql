-- ============================================================================
-- AutoBid Mobile - Migration 00112: Add allows_installment to auctions & listing_drafts
-- Allows sellers to indicate they accept installment payments during listing creation
-- ============================================================================

-- Add column to auctions table
ALTER TABLE auctions
ADD COLUMN IF NOT EXISTS allows_installment BOOLEAN NOT NULL DEFAULT FALSE;

-- Add column to listing_drafts table
ALTER TABLE listing_drafts
ADD COLUMN IF NOT EXISTS allows_installment BOOLEAN NOT NULL DEFAULT FALSE;

-- Update the submit_listing_from_draft RPC to propagate the new field
...
BEGIN
  ...
  -- Prepare snipe guard configuration
  ...
  -- Prepare installment flag
  v_allows_installment := COALESCE(v_draft.allows_installment, FALSE);
  ...
  -- Insert auction with installment flag
  ...
  -- Mark draft as submitted (soft delete)
