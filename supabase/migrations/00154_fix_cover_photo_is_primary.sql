-- ============================================================================
-- Migration 00154: Fix cover photo is_primary in submit_listing_from_draft
-- ============================================================================
-- ROOT CAUSE: The function in migration 00149 inserts photos into auction_photos
-- WITHOUT setting is_primary. The Dart _syncSelectedPrimaryPhoto post-step also
-- fails because the function soft-deletes the draft BEFORE Dart can read
-- cover_photo_url (RLS blocks SELECT on deleted drafts).
--
-- FIX: Set is_primary inline during photo INSERT, comparing each photo_url
-- against v_draft.cover_photo_url. If no cover_photo_url is set, all photos
-- default to is_primary=FALSE and the view falls back to first by display_order.
-- ============================================================================
...
BEGIN
  ...
  -- ========================================================================
  -- STEP 5: Copy photos from draft to auction (with is_primary)
  -- ========================================================================
  IF v_draft.photo_urls IS NOT NULL THEN
    ...
    LOOP
      ...
      LOOP
        INSERT INTO auction_photos (auction_id, photo_url, category, display_order, is_primary)
        VALUES (
          v_auction_id, v_photo_url, v_photo_category, v_display_order,
          COALESCE(v_photo_url = v_draft.cover_photo_url, FALSE)
        );
  ...
  -- If no photo matched cover_photo_url, default first photo as primary
  IF NOT EXISTS (
    SELECT 1 FROM auction_photos
    WHERE auction_id = v_auction_id AND is_primary = TRUE
  ) THEN
